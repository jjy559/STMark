...
/*    public static void main(String[] args) throws CsvValidationException, SQLException, IOException {
    ...
    public static void altAttackwithPK2(Data data,String databaseName,double rate) throws SQLException, CsvValidationException, IOException {
        ModifiedData d = new ModifiedData(data);//read in data
        HashSet<Integer> set = select_tuple(d,1-rate);//The tuples to be modified
        Random random = new Random();
        for(int i : set){
    ...
    static HashSet<Integer> select_insetred_tuple(ModifiedData d, double rate,int num){
        //In deletion attacks, the proportion indicated for deletion in the deletion attack,
        //In modification attacks, the proportion indicated for not modified in the modification attack

        //tuple_num is the number of tuples retained in the deletion attack and the number of tuples modified in the modification attack.
        int tuple_num = (int)(rate*num);

        HashSet<Integer> set = new HashSet<>();
        Random random = new Random();
        while(set.size()<tuple_num){
            int temp = num+random.nextInt(d.d.get(0).size()-num);
            set.add(temp);
        }
        return set;
    }
    ...
    public static void insertAttackWithoutSQL(String databaseName,double rate,int num,int[] Attrs) throws SQLException, CsvValidationException, IOException {
        ModifiedData d = new ModifiedData("dataset\\"+databaseName,'a',1000000,Attrs);//read in data
        int tmp = (int)(num * rate);//The number of tuples to be inserted
        ...
        for(int i=0;i<num;i++){//Copy the original database into the attacked database
            for(int j = 0;j<d.d.size();j++){
                datatmp.d.get(j).add(d.d.get(j).get(i));
            }
        }

        HashSet<Integer> set = select_insetred_tuple(d,rate,num);
    ...
    public static void alterAttack(String databaseName,double rate,int[] Attrs,int num) throws SQLException, IOException, JWNLException, CsvValidationException, JWNLException {
        ModifiedData d = new ModifiedData("dataset\\"+databaseName,'a',num,Attrs);//read in data
        HashSet<Integer> set = select_tuple(d,rate);
    ...
    public static void alterAttack2(String databaseName,double rate,int[] Attrs,int num) throws SQLException, IOException, JWNLException, CsvValidationException, JWNLException {
        ModifiedData d = new ModifiedData("dataset\\"+databaseName,'a',num,Attrs);//read in data
        HashSet<Integer> set = select_tuple(d,rate);
        SemanticUtils utils = new SemanticUtils();
        for(int i : set){//Delete all tuples except for the select
            ...
            utils.synonymSubstitution2(d,i);
            d.d.get(0).set(i,Util.getRandomAlphaString(10));
            d.d.get(1).set(i,Util.getRandomAlphaString(10));
...
class ModifiedData{
    ...
    public ModifiedData(String str, char ch, int num,int[] Attrs) throws SQLException, IOException, CsvValidationException {
        Reader reader = Files.newBufferedReader(Paths.get(str));
        CSVReader csvReader = new CSVReader(reader);
        String[] record;
        record = csvReader.readNext();
